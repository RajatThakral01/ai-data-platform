...
def _build_prompt(df: pd.DataFrame, question: str, prev_error: str | None = None) -> str:
    ...
    prompt = (
        "You are a data analyst assistant. Generate ONLY executable Python code "
        "with no explanation. The result must be stored in a variable called `result`.\n\n"
        "STEP 1 — DECIDE WHICH PATH TO TAKE:\n"
        "Before writing any code, decide whether this question requires actual computation:\n\n"
        "PATH A — Computational (use pandas):\n"
        "  Use this path if the question requires computing, aggregating, filtering,\n"
        "  counting, comparing, or looking up actual values FROM the DataFrame.\n"
        "  Examples: 'what is the average monthly charges', 'how many customers churned',\n"
        "  'which contract type has the highest revenue', 'show me the top 5 products'\n"
        "  → Write pandas code as normal. Store the computed result in `result`.\n\n"
        "PATH B — Conceptual / Definitional (skip pandas entirely):\n"
        "  Use this path if the question is general knowledge, definitional, or conceptual\n"
        "  and does NOT require touching any actual data values from the DataFrame.\n"
        "  Examples: 'what does churn mean', 'why does monthly charges matter',\n"
        "  'what is customer lifetime value', 'explain what tenure represents'\n"
        "  → Do NOT reference `df` at all. Just write ONE line:\n"
        "    result = \"Your plain-English answer here.\"\n"
        "  The answer should be clear, business-focused, and 2-4 sentences.\n\n"
        "IMPORTANT RULES:\n"
