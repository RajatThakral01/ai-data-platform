...
from modules.data_insights import render_visualisation_page  # noqa: E402
from modules.eda import render_dataset_summary  # noqa: E402
...
def _handle_upload(uploaded) -> None:
    ...
    try:
        ...
        # RAG Auto-indexing
        rag_key = f"rag_indexed_{uploaded.name}"
        if not st.session_state.get(rag_key):
            with st.spinner("Indexing dataset for semantic search..."):
                try:
                    ...
                    st.sidebar.warning(f"Could not index dataset for RAG: {e}")
...
def _render_distributions(results: dict) -> None:
    ...
    if selected:
        fig = plots[selected]
        if fig is None:
            st.info("This column has only 1 unique value — distribution chart not meaningful")
        else:
            st.plotly_chart(fig, use_container_width=True)
...
def _nlq_ai_mode(df: pd.DataFrame) -> None:
    ...
    try:
        ...
        indexed_datasets = vector_store.list_indexed_datasets()
    except Exception:
        indexed_datasets = []
    ...
    with st.sidebar:
        ...
        if indexed_datasets:
            ...
            # Changed logic slightly to make sure it confirms user intent if needed, or just clear active.
            if st.button("🚨 Clear Index", help="Delete current dataset index from ChromaDB"):
                current_ds = st.session_state.get('uploaded_name')
                if current_ds:
                    if vector_store.delete_dataset(current_ds):
                        st.session_state.pop(f"rag_indexed_{current_ds}", None)
                        st.success(f"Cleared index for {current_ds}")
                        st.rerun()
                else:
                    st.warning("No dataset currently active.")
        else:
            st.info("No datasets indexed yet.")

    cross_ds = st.toggle("Search across all datasets", value=False)
    ...
    if st.button("🚀  Ask", type="primary", use_container_width=True):
        ...
        chunks = out.get("context_chunks", [])
        if chunks:
            with st.expander("🔍 View Retrieved Context"):
                for idx, chunk in enumerate(chunks):
                    doc = chunk.get("document", "")
                    src = chunk.get("source", "unknown")
                    st.markdown(f"**Chunk {idx+1} [Source: {src}]**:\n`{doc}`")
